...
CAST(A.RMSDTCHGDT AS DATE) AS RMSDTCHGDT,
A.WFORGPRBAL
FROM HSFLCLNTWF A
...
A.HALIEN AS 'Lien Flag',
(CASE WHEN A.WFLIENDT = '0001-01-01' THEN NULL ELSE A.WFLIENDT END) AS 'Date Lien',
CAST(A.RMSDTCHGDT AS DATE) AS RMSDTCHGDT,A.WFORGPRBAL
FROM HSFLCLNTWF A
LEFT JOIN RMSPSYSCDE B ON A.PYMT_TRAN_CD = B.SYSCODETYP AND B.RMSRECTYPE = 1
LEFT JOIN HUBRELBAGD C ON A.ACCT_NUM = C.RACCTNUM
LEFT JOIN RMRMCLNM_REQ_DATA D ON A.ORGCODE = D.RMSBRGLVL2
...
CAST(A.LSTCMMTDTE AS DATE) AS 'Last Comment Dt', A.MSTPOENAM1 AS 'Employer Name', CAST(A.LSTSTATCHG AS DATE) AS 'Last Status Dt', A.LSTSSTATUS AS 'Last Status Code',CAST(A.RMSDTCHGDT AS DATE) AS RMSDTCHGDT,A.RMSCHGAMT
FROM RMSPMASTRP A
