...
CASE
    WHEN A.RMSDTCHGDT = '00000000' --AH06052026 Start
         OR A.RMSDTCHGDT IS NULL
         OR A.RMSDTCHGDT = ''
    THEN NULL
    ELSE STR_TO_DATE(A.RMSDTCHGDT, '%Y%m%d') --AH06052026 Emd
END AS RMSDTCHGDT,
A.WFORGPRBAL
FROM HSFLCLNTWF A
...
CASE
    WHEN A.RMSDTCHGDT = '00000000' --AH06052026 Start
         ...
         OR A.RMSDTCHGDT = ''
    THEN NULL
    ELSE STR_TO_DATE(A.RMSDTCHGDT, '%Y%m%d')  --AH06052026 End
END AS RMSDTCHGDT,A.WFORGPRBAL
FROM HSFLCLNTWF A
LEFT JOIN RMSPSYSCDE B ON A.PYMT_TRAN_CD = B.SYSCODETYP AND B.RMSRECTYPE = 1
LEFT JOIN HUBRELBAGD C ON A.ACCT_NUM = C.RACCTNUM
LEFT JOIN RMRMCLNM_REQ_DATA D ON A.ORGCODE = D.RMSBRGLVL2
...
CAST(A.LSTCMMTDTE AS DATE) AS 'Last Comment Dt', A.MSTPOENAM1 AS 'Employer Name', CAST(A.LSTSTATCHG AS DATE) AS 'Last Status Dt', A.LSTSSTATUS AS 'Last Status Code',CASE
    WHEN A.RMSDTCHGDT = '00000000'  --AH06052026 Start
         OR A.RMSDTCHGDT IS NULL
         OR A.RMSDTCHGDT = ''
    THEN NULL
    ELSE STR_TO_DATE(A.RMSDTCHGDT, '%Y%m%d') --AH06052026 End
END AS RMSDTCHGDT,A.RMSCHGAMT
FROM RMSPMASTRP A
